...
if fx_container == 0 then
    ...
    if active_fx == -1 then
        ...
        -- Calculate the next FX within the range inside the container
        local next_fx = active_fx + 1
        if next_fx > max_fx then
            next_fx = min_fx  -- Return to the first in the range if we reach the last one
        end
...
if apply_to_all_plugins then
    script_name = "Cycle_Select-Bypass_AllPlugins_" .. track_name
    if fx_container > 0 then
        script_name = script_name .. "_" .. fx_container .. "Container"
    end
    script_name = script_name .. ".lua"
else
    script_name = "Cycle_Select-Bypass_Plugins_range(" .. (min_fx + 1) .. "-" .. (max_fx + 1) .. ")_" .. track_name
    if fx_container > 0 then
        script_name = script_name .. "_" .. fx_container .. "Container"
    end
    script_name = script_name .. ".lua"
